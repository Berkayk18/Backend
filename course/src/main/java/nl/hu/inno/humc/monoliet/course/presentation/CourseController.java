...
public class CourseController {
    ...
    public ResponseEntity<?> getCourseByTitle(@PathVariable String id) {
        var courseId = Long.parseLong(id);
        Optional<CourseDTO> course = this._courseService.getCourseById(courseId);
    ...
    @GetMapping(value = "{id}/to-delete-course", produces = "application/json;charset=UTF-8")
    public ResponseEntity<String> getCourseToRemoveByTitle(@PathVariable String id) {
        var courseId = Long.parseLong(id);
        String errorMessage = this._courseService.getCourseToRemoveById(courseId);
        if (errorMessage == null || errorMessage.isEmpty()) {
            return ResponseEntity.ok("Course with id " + id + " is successfully deleted");
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(errorMessage);
    }

    @DeleteMapping(value = "{id}", produces = "application/json;charset=UTF-8")
    public ResponseEntity<String> removeCourseByTitle(@PathVariable String id) {
        var courseId = Long.parseLong(id);
        String errorMessage = this._courseService.removeCourseById(courseId);
        if (errorMessage == null || errorMessage.isEmpty()) {
            return ResponseEntity.ok("Course with id " + id + " is successfully deleted");
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(errorMessage);
